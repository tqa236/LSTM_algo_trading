...
def train_one_ticker(dataset, model_name, timestep=240, feature=31,
                     dropout_level=0.1):
    """Train an LSTM model with 1 feature."""
    for i in range(len(dataset[0])):
        for j in range(feature):
            model_period = f"{model_name}_period{i}_ticker{j}.h5"
            ...
            x_final = np.reshape(
                x_final, (x_final.shape[0], x_final.shape[1], 1))
...
def main():
    ...
    train_one_ticker(dataset, args.outdir)
    print("Done.")
    return 0
